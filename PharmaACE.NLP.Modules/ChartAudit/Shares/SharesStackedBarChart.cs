...
{
    public class SharesStackedBarChart : CAChartBase
    {
        public List<StackedBar> StackedBars { get; set; }

        public SharesStackedBarChart()
        {
            ...
            foreach (var row in dataSlices)
            {
                ...
                if (!String.IsNullOrWhiteSpace(tumorName))
                {
                    ...
                    double measureVal = row.RecognizedEntities.
                        ...
                        FirstOrDefault();
                        stackedBar.Stacks.Add(new Ordinate { Legend = regimenName, Value = measureVal });
                        if (!legends.Contains(regimenName))
                            legends.Add(regimenName);
