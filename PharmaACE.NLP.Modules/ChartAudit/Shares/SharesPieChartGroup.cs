...
{
    public class SharesPieChartGroup : CAChartBase
    {
        public List<PieChart> PieCharts { get; set; }

        public SharesPieChartGroup()
        {
        ...
        private List<PieChart> Populate(List<SentenceFragment> dataSlices, string monthYear)
        {
            ...
            if (RegimenNames.Distinct().Count() == 1)
            {
                ...
                FirstOrDefault();
                    if (String.Compare(monthYear, sliceMonthYear, true) == 0)
                    {
                        ...
                        pieCharts[0].Slices.Add(new PieSlice { Legend = RegimenNames[0]/*tumorName*/, Value = measureVal });
                    }
                }
                if (residualVal > 0)
            ...
            {
                pieCharts = new List<PieChart> { new PieChart { Caption = String.Format("{0} {1} in {2}", TumorNames[0],
            ...
            FirstOrDefault();
                ...
                FirstOrDefault();
                        residualVal -= measureVal;
                        pieCharts[0].Slices.Add(new PieSlice { Legend = regimenName, Value = measureVal });
                    }
                }
